...
const apiLimiter = rateLimit({
  ...
  // Don't count Railway's healthchecks against the per-IP quota -- they hit
  // /api/health every ~30s from a fixed Railway IP, and would otherwise
  // share that bucket with any real users behind the same NAT.
...
// Extract the query string portion of a request URL, including the leading '?'.
// Uses originalUrl so middleware mounting (current or future) doesn't strip it.
 ...
 *
 * 0x v2 swap endpoints are GET-only (price + permit2 quote). The POST handler
 * is kept for forward-compat in case 0x adds POST endpoints later -- it's a
 * passthrough and 0x will reject anything it doesn't support with its own 4xx.
 * ========================================================================= */
 ...
 * cache and back off on our side. Most quote endpoints are GET, but
 * /v1/advanced/routes is POST, so support both.
 *
 ...
 *                usdPrice, usdValue, pct24h, isNative }] }
 * ========================================================================= */
  ...
  try {
    ...
    results.forEach((settled) => {
      ...
      items.forEach((t) => {
        ...
        // Decimals: handle 0 (valid for some tokens), null, undefined, string,
        // and number. parseInt(t.decimals || '18') would wrongly turn 0 into 18.
        const rawDec = t.decimals;
        const decimals = rawDec != null && Number.isFinite(Number(rawDec))
          ? Number(rawDec)
          : 18;
...
// We deliberately do NOT process.exit() here -- Railway will restart the
// container on crash, but we'd rather keep serving healthy requests if at
// all possible. If you start seeing repeated unhandledRejections in the
// logs, treat that as a real bug to track down, not noise.
