 ...
 * /api/jupiter/*      - Jupiter read-only market/token data used by frontend
...
/* -- JUPITER READ-ONLY PROXY ------------------------------------------------ */
/*
 * This stays read-only and is used by App.js for:
 * - Markets page data
 * - Verified Solana token registry
 * - Optional price lookup by child components
 *
 * Swaps are handled by OKX, not Jupiter.
 */
const JUPITER_ALLOWED_ENDPOINTS = new Set([
...
async function proxyJupiter(req, res) {
  try {
    ...
    if (!JUPITER_ALLOWED_ENDPOINTS.has(subPath)) {
      return res.status(404).json({ error: 'Jupiter endpoint not allowed: ' + subPath });
    }
    ...
    if (e.name === 'AbortError') {
      return res.status(504).json({ error: 'Jupiter request timed out' });
    }
    logError('jupiter', e);
    return res.status(500).json({ error: e.message || 'Unknown error' });
  }
...
async function proxyLifi(req, res) {
  try {
    ...
    const response = await fetchWithTimeout(url, fetchOpts, 12_000);
    return respondJsonOrError(res, response, await safeJson(response));
  } catch (e) {
    if (e.name === 'AbortError') {
      return res.status(504).json({ error: 'LiFi request timed out' });
    }
    logError('lifi', e);
    return res.status(500).json({ error: e.message || 'Unknown error' });
  }
...
app.listen(PORT, () => {
  ...
  if (!OKX_FEE_WALLET_SOL) {
    console.warn('  WARNING: OKX_FEE_WALLET_SOL not set - Solana fees not collected');
  }
  if (!OKX_FEE_WALLET_EVM) {
    console.warn('  WARNING: OKX_FEE_WALLET_EVM not set - EVM fees not collected');
  }
