Goes at src/lib.rs in your Playground project:
...
// =====================================================================
// SUPER ADMIN — hardcoded. Absolute control. Receives all treasury fees.
// Cannot change without redeploying the program.
// =====================================================================
...
// CONSTANTS
// =====================================================================
const FEE_BPS: u64 = 2_500;                 // 25%
const BPS_DIVISOR: u64 = 10_000;
const BETTING_DURATION: i64 = 360;          // 6 minutes
const GAP_DURATION: i64 = 30;               // 30 sec gap after close
const MAX_FUTURE_ROUNDS: u64 = 20;          // bet up to 20 rounds ahead
const FORCE_REFUND_DELAY: i64 = 86_400;     // 24h
const CLAIM_FORFEIT_DELAY: i64 = 259_200;   // 72h — unclaimed sweep
...
pub mod flipsy {
    ...
    pub fn initialize(
        ...
        c.admin = admin;
        c.cranker = cranker;
        c.pyth_feed = ctx.accounts.pyth_feed.key();
        c.current_epoch = 0;
        c.paused = false;
        c.min_bet = min_bet;
        c.max_bet = max_bet;
    ...
    pub fn set_pyth_feed(ctx: Context<AdminOrSuper>, new_feed: Pubkey) -> Result<()> {
...
fn read_pyth_price(feed_info: &AccountInfo, clock: &Clock) -> Result<i64> {
    let price_feed =
        load_price_feed_from_account_info(feed_info).map_err(|_| FlipsyError::PythError)?;
