 ...
 *         ConnectionProvider      -- Solana RPC (server proxy, key never reaches browser)
 ...
 *
 * Solana RPC routing:
 *   The browser ALWAYS hits our backend proxy at /api/solana-rpc. The
 *   server then forwards to Helius (or fallback) using HELIUS_API_KEY
 *   set on Railway -- the key NEVER leaves the server. This blocks the
 *   "view-source -> extract API key -> drain quota" attack that direct
 *   REACT_APP_HELIUS_API_KEY embedding allows.
 */
...
/* ============================================================================
 * ENV: REOWN / WALLETCONNECT, PRIVY APP ID
 * ========================================================================= */
...
/* ============================================================================
 * SOLANA RPC -- ALWAYS go through the server proxy.
 *
 * The server (server.js) reads HELIUS_API_KEY / HELIUS_RPC_URL from its own
 * env and forwards JSON-RPC calls to Helius. The key never reaches the
 * browser, so it can't be extracted via view-source.
 *
 * In dev, package.json's "proxy" field forwards /api/* from CRA (port 3000)
 * to the express server (port 3001). In prod (Railway), both serve from the
 * same origin so /api/solana-rpc resolves locally.
 *
 * Connection from @solana/web3.js requires an absolute URL, so we build
 * it from window.location.origin at runtime. SSR fallback is a no-op since
 * this app is SPA-only.
 *
 * Note: this disables WebSocket subscriptions (onAccountChange, onSignature,
 * etc.) since the proxy is HTTP-only. The app polls instead, which it was
 * already doing. If you ever need subscriptions, route them through a
 * dedicated WS proxy or use a public Solana WS endpoint.
 * ========================================================================= */
 ...
 *
 * Solana cluster routed through our /api/solana-rpc proxy so embedded
 * wallet RPC calls share the same endpoint as the rest of the app and
 * the Helius key stays server-side.
 *
