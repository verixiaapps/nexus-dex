...
import { PLATFORM_NAME, PLATFORM_FEE_DISPLAY } from ‘./config’;
...
```
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 16, flexShrink: 0 }}>
        {coins.slice(0, 3).map(c => (
          <span key={c.id} style={{ display: 'flex', gap: 5, fontSize: 11, fontFamily: "'JetBrains Mono', monospace", whiteSpace: 'nowrap', color: C.muted }}>
            <span style={{ color: '#fff', fontWeight: 700 }}>{c.symbol?.toUpperCase()}</span>
      ...
      <div style={{ flexShrink: 0 }}>
        <WalletMultiButton style={{
          background: connected ? 'rgba(0,255,163,.1)' : 'linear-gradient(135deg,#00e5ff,#0055ff)',
          border: connected ? '1px solid rgba(0,255,163,.3)' : 'none',
          borderRadius: 10, padding: '8px 20px',
          color: connected ? C.green : C.bg,
          fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 13,
          height: 'auto', lineHeight: 1.4,
        }} />
      </div>
    </div>
  </header>
  ...
  <main style={{ position: 'relative', zIndex: 1, maxWidth: 1400, margin: '0 auto', padding: '36px 24px 80px' }}>
    ...
    {tab === 'markets' && <Markets coins={coins} loading={loading} onSelectCoin={(id) => setTab('swap')} />}
    {tab === 'buy' && <BuyCrypto coins={coins} walletAddress={publicKey?.toString()} />}
  ...
  <footer style={{ borderTop: `1px solid ${C.border}`, padding: '20px 24px' }}>
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {['JUPITER ROUTING', 'RANGO CROSS-CHAIN', `${PLATFORM_FEE_DISPLAY} FEE`, 'NON-CUSTODIAL'].map(label => (
      ...
      <span style={{ fontSize: 11, color: '#2e3f5e', fontFamily: "'JetBrains Mono', monospace" }}>
        © 2025 {PLATFORM_NAME} · Prices via CoinGecko
      </span>
    </div>
  </footer>
</div>
