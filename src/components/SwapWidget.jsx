...
- 1. ONE quote shown to user. Internal route picker is silent.
- 1. Header network selector = DESTINATION chain. Source auto-picks.
- 1. Cross-chain works for any pair. LiFi primary, multi-hop fallback.
- 1. User stays on swap.verixiaapps.com. No external redirects.
- 1. Buy mode: from = native of source chain, to = the token.
- ```
   Sell mode: from = the token, to = native of header chain.
  ```
- 1. Quick-buy presets ($) + Sell presets (%) appear for every token.
   */
...
- 1. Decompile, append SystemProgram.transfer for our fee
- 1. Recompile, user signs ONCE, broadcast
- 
- 0x (EVM same-chain) with Permit2:
- 1. Get permit2 quote — includes EIP-712 typed data + transaction
- 1. User signs typed data (off-chain, no popup-2, no gas)
- 1. We attach signature + length prefix to the calldata
- 1. User signs the resulting tx ONCE, broadcast
- (Fee is built into 0x quote via swapFeeBps + swapFeeRecipient)
- 
- LiFi (cross-chain):
- 1. Get fresh quote with real addresses
- 1. Solana side: deserialize, send (fee already included via integrator)
- 1. EVM side:
- ```
