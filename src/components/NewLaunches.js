    ...
    return (
      ...
      >
        ...
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          ...
          {change != null && (
            <div style={{ fontSize: 11, fontWeight: 600, color: change >= 0 ? C.green : C.red }}>
              {pctFmt(change)}
            </div>
          )}
        </div>
  ...
  return (
    <div
      ...
      style={{
        ...baseStyle,
        display: 'grid',
        gridTemplateColumns: '28px minmax(0,1fr) 120px 100px 80px',
        gap: 8,
        alignItems: 'center',
      }}
    >
      ...
      <div style={{ textAlign: 'right' }}>
        {change != null && (
          <div style={{ fontSize: 12, fontWeight: 600, color: change >= 0 ? C.green : C.red }}>
            {pctFmt(change)}
          </div>
        )}
      </div>
...
export default function Trending({ onSelectCoin }) {
        ...
        if (j.code === '0' && Array.isArray(j.data)) {
          const mapped = j.data
            .map(t => ({
              ...
              change24h: t.market?.priceChange24h != null ? Number(t.market.priceChange24h) : null,
          ...
          setError(j.msg || 'Failed to load trending tokens');
        }
        setLoading(false);
      } catch {
        if (!cancelled) {
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', width: '100%', boxSizing: 'border-box', paddingBottom: 'calc(env(safe-area-inset-bottom) + 80px)' }}>
      <div style={{ marginBottom: 16 }}>
        ...
        <p style={{ color: C.muted, fontSize: 12, marginTop: 4 }}>
          Top pump tokens via OKX · {tokens.length} tokens
        </p>
      </div>
      {loading ? (
        ...
        <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 16, overflow: 'hidden' }}>
          {!isMobile && (
            <div style={{ display: 'grid', gridTemplateColumns: '28px minmax(0,1fr) 120px 100px 80px', gap: 8, padding: '10px 16px', borderBottom: '1px solid rgba(0,229,255,.06)', fontSize: 10, color: C.muted, fontWeight: 700, letterSpacing: 0.8 }}>
              <div>#</div>
              <div>NAME</div>
              <div style={{ textAlign: 'right' }}>MC / VOL</div>
              <div style={{ textAlign: 'right' }}>BUYS/SELLS</div>
          ...
          {tokens.map((c, i) => (
            <Row key={c.id || c.mint || i} c={c} i={i} isMobile={isMobile} onClick={handleClick} />
          ))}
        </div>
      )}
    </div>
