...
function PumpDrawer({ open, onClose, mode, token, solPrice, onConnectWallet, isConnected, presets, onPresetsChange, presetUsd }) {
  ...
  // FEE MATH (locked 5%, full spread to SOL_FEE_WALLET).
  // Buy: user spends solAmt total. PumpPortal trades 95% of it. The
  // remaining 5% becomes the fee transfer instruction in the same tx.
  // Sell: PumpPortal returns SOL to the user; we then transfer 5% of
  // the *estimated* SOL out to the fee wallet. The 0.85 cushion makes
  // the fee survivable even if slippage eats up to 15% of the receive.
  var totalLamports = Math.floor(solAmt * LAMPORTS_PER_SOL);
  var buyFeeLamports = Math.floor(totalLamports * PLATFORM_FEE_RATE);
  var buyTradeLamports = totalLamports - buyFeeLamports;
  var buyTradeSolAmount = buyTradeLamports / LAMPORTS_PER_SOL;
  ...
  var executeTrade = async function() {
    ...
    try {
      ...
      // Compile -> decompile -> append fee transfer -> recompile.
      // Single signature, single transaction. Fee goes 100% to
      // SOL_FEE_WALLET (no co-signers, no splits).
...
function TokenCard({ token, onCardClick, onBuyClick, onSellClick, onQuickBuy, isNew, solPrice }) {
  ...
  // and trigger Privy login. After login completes, useEffect auto-resumes
  // the action -- no need to tap again.
  ...
  // Auto-resume pending intent after Privy login completes.
  ...
  async function handleQuickBuy(e, usd) {
    ...
    // Connect-first: trigger Privy directly (skip full wallet modal).
    if (!anyConnected) {
...
export default function NewLaunches({ coins, jupiterTokens, onConnectWallet, isConnected, isSolanaConnected, walletAddress, resetKey }) {
  ...
  var updateTokenDexData = useCallback(function(dexData) {
    ...
    tokensRef.current = tokensRef.current.map(function(t) {
      ...
      return Object.assign({}, t, d, { mint: t.mint, priceHistory: newHistory, pct1h: d.pct1h != null ? d.pct1h : pct1h, pct5m: d.pct5m != null ? d.pct5m : pct5m });
    });
  ...
  var queueDexFetch = useCallback(function(mint) {
    ...
    dexTimerRef.current = setTimeout(async function() {
      ...
      var data = await fetchGeckoTerminal(batch);
  ...
  var addToken = useCallback(function(token) {
    ...
    fetchGeckoTerminal([token.mint]).then(function(data) {
      ...
      tokensRef.current = tokensRef.current.map(function(t) {
        ...
        return Object.assign({}, t, d, { mint: t.mint, priceHistory: d.price > 0 ? [d.price] : [] });
      });
      setTokens([].concat(tokensRef.current));
    });
  ...
  useEffect(function() {
      ...
      try {
        ...
        ws.onmessage = function(event) {
          try {
            ...
            var looksLikeCreate = false;
            for (var li = 0; li < logs.length; li++) {
              var line = logs[li];
              if (line && (line.indexOf('Instruction: Create') !== -1 ||
                           line.indexOf('Instruction: InitializeMint') !== -1 ||
                           line.indexOf('Instruction: Initialize2') !== -1)) {
                looksLikeCreate = true;
                break;
              }
            }
            ...
            var t1 = setTimeout(async function() {
              ...
              if (!alive) return;
              var mintAddr = await extractMintFromTx(connection, sig);
              if (!alive) return;
              if (!isValidMint(mintAddr)) return;
    ...
    if (cached.length > 0) {
      cached = cached.filter(function(t) { return isValidMint(t && t.mint); });
    ...
    refreshTimerRef.current = setInterval(async function() {
      if (!alive) return;
      var mints = tokensRef.current.slice(0, 30).map(function(t) { return t.mint; }).filter(isValidMint);
      if (!mints.length) return;
      var data = await fetchGeckoTerminal(mints);
      if (!alive) return;
      updateTokenDexData(data);
    ...
    var loadInitial = async function() {
      try {
        ...
        var initialTokens = pools.map(function(pool) {
          ...
          if (!isValidMint(mintAddr)) return null;
        ...
        fetchGeckoTerminal(initialTokens.map(function(t) { return t.mint; })).then(function(gtData) {
          ...
          tokensRef.current = tokensRef.current.map(function(t) {
            var d = gtData[t.mint];
            if (!d) return t;
            return Object.assign({}, t, { mint: t.mint, image: d.image || t.image, name: d.name || t.name, symbol: d.symbol || t.symbol, price: d.price || t.price, marketCap: d.marketCap || t.marketCap, pct5m: d.pct5m !== null ? d.pct5m : t.pct5m, pct1h: d.pct1h !== null ? d.pct1h : t.pct1h, pct24h: d.pct24h !== null ? d.pct24h : t.pct24h });
          });
          setTokens([].concat(tokensRef.current));
          saveCachedTokens(tokensRef.current);
        });
      ...
      <div style={{ marginBottom: 18 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: wsStatus === 'live' ? 'rgba(0,255,163,.08)' : 'rgba(255,149,0,.08)', border: '1px solid ' + (wsStatus === 'live' ? 'rgba(0,255,163,.2)' : 'rgba(255,149,0,.2)'), borderRadius: 20, padding: '3px 10px' }}>
            ...
            <span style={{ fontSize: 10, color: wsStatus === 'live' ? C.green : C.orange, fontWeight: 600 }}>{wsStatus === 'live' ? 'LIVE' : wsStatus === 'reconnecting' ? 'RECONNECTING' : wsStatus === 'offline' ? 'POLLING' : 'CONNECTING'}</span>
          </div>
        </div>
