...
import { Raydium, LAUNCHPAD_PROGRAM, TxVersion, getPdaLaunchpadConfigId } from '@raydium-io/raydium-sdk-v2';
...
/* ============================================================================
* PINATA UPLOADS -- routed through our own server, NOT direct to pinata.cloud.
*
* The previous version sent `REACT_APP_PINATA_JWT` from the browser. CRA
* inlines REACT_APP_* into the public bundle at build time, so the JWT was
* sitting in the static JS file shipped to every visitor -- anyone could
* grab it and pin arbitrary content under your account.
*
* server.js exposes /api/pinata/json and /api/pinata/file for this exact
* reason; the JWT only lives on Railway in PINATA_JWT (no REACT_APP_ prefix).
* ========================================================================= */
...
async function uploadMetadata(name, symbol, description, imageUri) {
 ...
 } catch (e) { /* fall through to data: URI fallback */ }
 // Fallback when the server proxy is unavailable -- returns a base64 data:
 // URI so the launch can still proceed with off-IPFS metadata. The token
 // will work but metadata won't be permanent.
...
async function getActiveLaunchpadConfig(raydium) {
 ...
 // Static import (top of file) -- was previously a dynamic import inside the
 // function, which forced a separate webpack chunk for one tiny export and
 // delayed the fallback by a network round-trip in the failure case.
