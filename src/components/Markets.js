...
// Detect if user typed a contract address
var isContract = isValidMint(q.trim());

// Live search via Jupiter token list for name/symbol queries
useEffect(function() {
var trimmed = q.trim();
...
// Search all Solana tokens - Jupiter strict + DexScreener for full coverage
var qlLower = trimmed.toLowerCase();

// Fire Jupiter strict + DexScreener search in parallel
Promise.all([
  // Jupiter strict token list
  fetch('https://lite-api.jup.ag/tokens/v1/tagged/strict')
    .then(function(r) { return r.ok ? r.json() : []; })
    .catch(function() { return []; }),
  // DexScreener search - covers all tokens with trading pairs
  ...
  // DexScreener matches - fills gap for non-strict tokens
  var dexSeen = new Set();
  ...
  // Fetch Jupiter prices for tokens without prices
...
// Fetch prices for Jupiter search results
useEffect(function() {
