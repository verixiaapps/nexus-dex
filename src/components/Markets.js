...
const SEED_QUERIES = ['BONK', 'WIF', 'JUP', 'JTO', 'PYTH', 'BOME', 'WEN'];
...
async function searchDexScreener(query) { try { const res = await fetch('/api/dexscreener/latest/dex/search?q='+encodeURIComponent(query)); const data = await res.json().catch(()=>null); if(!data||!Array.isArray(data.pairs)) return []; const seen = new Set(); const tokens = []; for(const p of data.pairs) { const t = mapDexSearchPair(p); if(!t||!isUsable(t)) continue; const key = (t.mint||t.address||'').toLowerCase(); if(seen.has(key)) continue; seen.add(key); tokens.push(t); } return tokens; } catch { return []; } }
    ...
    <div style={{ maxWidth:900, margin:'0 auto', width:'100%', boxSizing:'border-box' }}>
      ...
      {loading ? (
        ...
        <div style={{ background:C.card, border:'1px solid '+C.border, borderRadius:16, overflow:'hidden' }}>
          {!isMobile && (
            <div style={{ display:'grid', gridTemplateColumns:'28px minmax(0,1fr) 110px 80px 120px', gap:8, padding:'10px 16px', borderBottom:'1px solid rgba(0,229,255,.06)', fontSize:10, color:C.muted, fontWeight:700, letterSpacing:.8 }}>
