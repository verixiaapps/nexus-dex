...
export default function Markets({ coins, loading, onSelectCoin }) {
  ...
  const sorted = [...filtered].sort((a, b) => dir * ((a[sort] || 0) - (b[sort] || 0)));
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', width: '100%', boxSizing: 'border-box', overscrollBehavior: 'none' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        ...
        <input
          ...
          style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 10, padding: '9px 14px', color: '#fff', fontFamily: 'Syne, sans-serif', fontSize: 13, outline: 'none', width: '100%', maxWidth: 220 }}
        />
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 16, overflow: 'hidden' }}>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                  {c.image
                    ...
                    : <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(0,229,255,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: C.accent, flexShrink: 0 }}>{c.symbol && c.symbol.charAt(0).toUpperCase()}</div>
                  }
          ...
          {sorted.length === 0 && (
            <div style={{ padding: '40px 20px', textAlign: 'center', color: C.muted, fontSize: 13 }}>No coins found matching your search.</div>
          )}
        </div>
      )}
    </div>
