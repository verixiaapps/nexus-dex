...
async function fetchTokenData(mint) {
  ...
  try {
    // 1. Price & basic info
    const priceRes = await fetch(`/api/okx/dex/market/price?chainIndex=501&tokenAddresses=${mint}`);
    ...
    // 2. Token basic info (market cap, volume)
    let marketCap = 0, volume24h = 0;
    try {
      const infoRes = await fetch(`/api/okx/dex/market/token/basic-info?chainIndex=501&tokenAddress=${mint}`);
      const infoJson = await infoRes.json();
      if (infoJson.code === '0' && infoJson.data) {
        const d = Array.isArray(infoJson.data) ? infoJson.data[0] : infoJson.data;
        marketCap = Number(d.marketCap || d.mcap || 0);
        volume24h = Number(d.volume24h || d.volume || 0);
      }
    } catch {}

    // 3. 7-day candles for chart
    let chartData = [];
    let change7d = 0;
    try {
      const candleRes = await fetch(`/api/okx/dex/market/candles?chainIndex=501&tokenAddress=${mint}&interval=1d&startTime=${sevenDaysAgo}&endTime=${now}`);
      const candleJson = await candleRes.json();
      if (candleJson.code === '0' && Array.isArray(candleJson.data) && candleJson.data.length > 0) {
        const candles = candleJson.data.sort((a, b) => (a.t || a.time || 0) - (b.t || b.time || 0));
        chartData = candles.map(c => ({
          time: new Date((c.t || c.time || 0) * 1000).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
          price: parseFloat(c.close || c.c || 0),
        }));
        const firstPrice = parseFloat(candles[0].open || candles[0].o || 0);
        const lastPrice = parseFloat(candles[candles.length - 1].close || candles[candles.length - 1].c || 0);
