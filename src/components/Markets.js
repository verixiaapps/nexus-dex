...
function Row({ c, i, isMobile, onClick }) {
  ...
  const baseStyle = { padding: isMobile ? '12px 14px' : '12px 16px', borderBottom: '1px solid rgba(255,255,255,.025)', cursor: 'pointer' };
    ...
    return (
      <div onClick={() => onClick(c)} style={{ ...baseStyle, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          ...
          {change != null && <div style={{ fontSize: 11, fontWeight: 600, color: change >= 0 ? C.green : C.red }}>{pctFmt(change)}</div>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div onClick={() => onClick(c)} style={{ ...baseStyle, display: 'grid', gridTemplateColumns: '28px minmax(0,1fr) 120px 100px 80px', gap: 8, alignItems: 'center' }}>
      ...
      <div style={{ textAlign: 'right', fontSize: 11, color: C.muted }}>{buys}B / {sells}S</div>
      <div style={{ textAlign: 'right' }}>
        {change != null && <div style={{ fontSize: 12, fontWeight: 600, color: change >= 0 ? C.green : C.red }}>{pctFmt(change)}</div>}
      </div>
    </div>
  );
}

export default function NewLaunches({ onSelectCoin }) {
        ...
        if (j.code === '0' && Array.isArray(j.data)) {
          setTokens(j.data
            .map(t => ({
              ...
              buys1h: Number(t.market?.buyTxCount1h || 0),
              sells1h: Number(t.market?.sellTxCount1h || 0),
              change24h: null,
            }))
            .filter(t => t.marketCap >= 5000)
  ...
  const handleClick = useCallback((row) => {
    ...
    navigate(`/token/${row.mint}`);
  }, [onSelectCoin, navigate]);
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', width: '100%', boxSizing: 'border-box', paddingBottom: 'calc(env(safe-area-inset-bottom) + 80px)' }}>
      <div style={{ marginBottom: 16 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: '#fff', margin: 0 }}>Trending</h1>
        <p style={{ color: C.muted, fontSize: 12, marginTop: 4 }}>Top pump tokens via OKX · {tokens.length} tokens</p>
      </div>
      {loading ? (
        ...
        <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 16, overflow: 'hidden' }}>
          {!isMobile && (
            <div style={{ display: 'grid', gridTemplateColumns: '28px minmax(0,1fr) 120px 100px 80px', gap: 8, padding: '10px 16px', borderBottom: '1px solid rgba(0,229,255,.06)', fontSize: 10, color: C.muted, fontWeight: 700, letterSpacing: 0.8 }}>
              <div>#</div><div>NAME</div><div style={{ textAlign: 'right' }}>MC / VOL</div><div style={{ textAlign: 'right' }}>BUYS/SELLS</div><div style={{ textAlign: 'right' }}>24H</div>
            </div>
          )}
