...
function TokenSelect({ selected, onSelect, jupiterTokens }) {
  ...
  var isValidAddress = function(str) { return str && str.length >= 32 && str.length <= 44 && /^[1-9A-HJ-NP-Za-km-z]+$/.test(str); };
  var lookupByAddress = async function(addr) {
    ...
    try {
      ...
      if (found) { setContractToken(found); }
        ...
        if (res.ok) { var data = await res.json(); setContractToken({ mint: data.address, symbol: data.symbol, name: data.name, decimals: data.decimals, logoURI: data.logoURI }); }
        else { setContractToken({ mint: addr, symbol: addr.slice(0, 6) + '...', name: 'Custom Token', decimals: 6 }); }
      }
    } catch (e) { setContractToken({ mint: addr, symbol: addr.slice(0, 6) + '...', name: 'Custom Token', decimals: 6 }); }
    ...
    var fetchBals = async function() {
      try {
        if (fromToken && fromToken.mint === 'So11111111111111111111111111111111111111112') {
          var solLam = await connection.getBalance(publicKey); setFromBalance(solLam / 1e9);
        ...
        if (toToken && toToken.mint === 'So11111111111111111111111111111111111111112') {
          var solLam2 = await connection.getBalance(publicKey); setToBalance(solLam2 / 1e9);
        ...
        <div style={{ background: C.card2, borderRadius: 12, padding: 14, border: '1px solid ' + C.border, marginBottom: 4 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {fromBalance != null && fromBalance > 0 && (
              <button onClick={function() { var fees = totalFee + SPREAD + 0.002; var maxAmt = fromToken && fromToken.symbol === 'SOL' ? Math.max(0, fromBalance - fees) : fromBalance; setFromAmt(maxAmt > 0 ? maxAmt.toFixed(fromToken && fromToken.decimals <= 2 ? 2 : 6) : '0'); }} style={{ background: 'rgba(0,229,255,.12)', border: '1px solid rgba(0,229,255,.25)', borderRadius: 6, padding: '3px 8px', color: C.accent, fontSize: 11, fontWeight: 700, cursor: 'pointer', flexShrink: 0 }}>MAX</button>
            )}
          </div>
