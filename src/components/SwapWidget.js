...
```
  ...
  // Get swap instructions - compose fee into same transaction
  ...
  if (instrData && instrData.swapInstruction && !instrData.error) {
    // Build single transaction: swap + fee
    ...
    // Fee as fallback separate tx
    try {
      var fbh = await connection.getLatestBlockhash('finalized');
      var feeTx = new Transaction(); feeTx.recentBlockhash = fbh.blockhash; feeTx.lastValidBlockHeight = fbh.lastValidBlockHeight; feeTx.feePayer = publicKey;
