    ...
    try {
      ...
      if (instrData && instrData.swapInstruction && !instrData.error) {
        ...
        if (instrData.addressLookupTableAddresses && instrData.addressLookupTableAddresses.length) {
          ...
          lookupTables = lutInfos.reduce(function(acc, info, i) {
        ...
        var msgV0 = new TransactionMessage({ payerKey: publicKey, recentBlockhash: bh.blockhash, instructions: allIxs }).compileToV0Message(lookupTables);
        ...
        var txFB = VersionedTransaction.deserialize(txBuf);
    ...
    <div style={{ width: '100%', maxWidth: 520, margin: '0 auto', boxSizing: 'border-box', overscrollBehavior: 'none' }}>
      ...
      <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 18, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[0.1, 0.5, 1.0].map(function(v) {
              return (
                <button key={v} onClick={function() { setSlip(v); }} style={{ padding: '3px 8px', borderRadius: 6, fontSize: 11, cursor: 'pointer', background: slip === v ? 'rgba(0,229,255,.15)' : 'transparent', border: '1px solid ' + (slip === v ? 'rgba(0,229,255,.4)' : C.border), color: slip === v ? C.accent : C.muted }}>
                  {v}%
                </button>
              );
            })}
        ...
        <div style={{ marginTop: 10 }}>
          ...
          {useCustomAddress && (
            <input value={customAddress} onChange={function(e) { setCustomAddress(e.target.value); }} placeholder="Paste Solana wallet address…" style={{ width: '100%', background: C.card2, border: '1px solid rgba(0,229,255,.2)', borderRadius: 10, padding: '10px 12px', color: C.accent, fontFamily: 'monospace', fontSize: 11, outline: 'none', marginTop: 8 }} />
          )}
        </div>
