...
// Pay with SOL, receive the token being viewed
...
```
  // Get swap instructions - fee in same tx
  ...
  if (instrData && instrData.swapInstruction && !instrData.error) {
    var deserializeIx = function(ix) { return { programId: new PublicKey(ix.programId), keys: ix.accounts.map(function(a) { return { pubkey: new PublicKey(a.pubkey), isSigner: a.isSigner, isWritable: a.isWritable }; }), data: Buffer.from(ix.data, 'base64') }; };
    var allIxs = [];
...
// If still 0, derive from quote output
