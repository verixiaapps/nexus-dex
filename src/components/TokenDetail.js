            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              ...
              {q && searchResults.length === 0 && (
                <div style={{ padding: 24, textAlign: 'center', color: C.muted, fontSize: 13 }}>
                  No tokens found. Paste contract address above.
                </div>
              )}
  ...
  var executeSwap = async function() {
    ...
    try {
      ...
      try {
        ...
        var feeSol = (parseFloat(fromAmt) * fromPrice * totalFee) / solPrice;
        if (feeSol > 0.000001) {
...
export default function TokenDetail({ coin, coins, jupiterTokens, onBack, onConnectWallet, isConnected, isSolanaConnected, walletAddress }) {
  ...
  if (coin && coin.symbol) {
    jupiterToken = POPULAR_TOKENS.find(function(t) {
  ...
  if (!jupiterToken && coin && coin.symbol && jupiterTokens && jupiterTokens.length > 0) {
    var found = jupiterTokens.find(function(t) {
      return t.symbol && t.symbol.toUpperCase() === coin.symbol.toUpperCase();
    });
