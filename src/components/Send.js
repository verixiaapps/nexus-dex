...
// Same decimals logic as TokenDetail and SwapWidget
...
function TokenSearchModal({ open, onClose, jupiterTokens }) {
  ...
  var close = function() {
    setQ(''); setContractAddr(''); setContractToken(null); setSearchResults([]);
    onClose(null);
  };
      ...
      <div style={{
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          ...
          {displayTokens.map(function(t) {
            return (
              <div key={t.mint} onClick={function() { onClose(t); setQ(''); }}
...
export default function Send({ coins, jupiterTokens, onConnectWallet, isConnected, isSolanaConnected, walletAddress }) {
  ...
  var handleSend = async function() {
    ...
    try {
      ...
      // Get correct decimals — same as SwapWidget/TokenDetail
      ...
      if (selectedToken.isNative || selectedToken.mint === SOL_TOKEN.mint) {
        ...
        if (feeUnits > 0) {
          try {
            ...
            if (!feeAtaInfo) {
              transaction.add(createAssociatedTokenAccountInstruction(publicKey, feeAta, feePubkey, mintPubkey));
            }
          } catch (e) {}
    ...
    <div style={{ maxWidth: 520, margin: '0 auto' }}>
      ...
      <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 20, padding: 20 }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input value={recipient} onChange={function(e) { setRecipient(e.target.value); }}
              ...
              {[0.25, 0.5, 0.75, 1].map(function(p) {
                return (
                  <button key={p} onClick={function() { setAmount((solBalance * p * 0.99).toFixed(6)); }} style={{
                    ...
                    background: 'transparent', border: '1px solid ' + C.border, color: C.muted, fontFamily: 'Syne, sans-serif',
        ...
        <button onClick={handleSend} disabled={status === 'loading'} style={{
          ...
          cursor: status === 'loading' ? 'not-allowed' : 'pointer', transition: 'all .3s', minHeight: 52,
        }}>
