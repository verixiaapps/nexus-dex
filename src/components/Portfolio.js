...
export default function Portfolio({ coins, jupiterTokens, onSend, onConnectWallet, isConnected, isSolanaConnected, walletAddress, refreshKey, onSelectToken }) {
 ...
 // Refs hold the latest helpers so fetchSolBalances doesn't change identity
 // every time coins/jupiterTokens refresh (every 30s from App.js). Without
 // this, the polling effect cleared and re-created its 30s interval on
 // every refresh tick, multiplying Solana RPC load. (M6)
 const getPriceRef     = React.useRef(null);
 const getTokenInfoRef = React.useRef(null);
 ...
 React.useEffect(function() { getPriceRef.current = getPrice; }, [getPrice]);
 React.useEffect(function() { getTokenInfoRef.current = getTokenInfo; }, [getTokenInfo]);
 ...
 var fetchSolBalances = useCallback(async function() {
   ...
   try {
     ...
     // C9 fix -- query BOTH legacy SPL Token program AND Token-2022 program.
     // Most new Solana launches use Token-2022 (transfer hooks, fees, etc.),
     // and querying only the legacy program made those holdings invisible.
     ...
     allAccounts.forEach(function(account) {
       try {
         ...
         if (uiAmount && uiAmount > 0.000001 && !seenMints.has(info.mint)) {
           ...
           // Use ref-backed helpers to avoid stale-closure / re-render loop.
     ...
     var getPriceFn = getPriceRef.current || function() { return 0; };
     var missingMints = holdings.filter(function(h) { return getPriceFn(h.symbol) === 0; }).map(function(h) { return h.mint; });
     ...
     holdings.sort(function(a, b) {
       var priceA = getPriceFn(a.symbol) || a.jupPrice;
       var priceB = getPriceFn(b.symbol) || b.jupPrice;
   ...
   setSolLoading(false);
 }, [publicKey, connection, lookupAddress]);  // M6 fix: dropped getPrice/getTokenInfo from deps (now read via refs)
