...
// Fetch all EVM token balances with prices via Moralis
useEffect(function() {
...
return fetch(‘https://deep-index.moralis.io/api/v2.2/wallets/’ + evmAddress + ‘/tokens?chain=’ + chainId + ‘&exclude_spam=true&exclude_unverified_contracts=false’, {
...
```
  ...
  holdings.sort(function(a, b) { return (b.uiAmount * getPrice(b.symbol)) - (a.uiAmount * getPrice(a.symbol)); });
  ...
  setTotalValue(total);
  // Note: EVM balance added to display separately via wagmi
} catch (e) {
...
// Use connected publicKey OR manual address
  ...
  {!publicKey && (
    <div style={{ background: C.card, border: '1px solid rgba(0,229,255,.15)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        />
        <button onClick={function() { setLookupAddress(manualAddress.trim()); }} style={{ background: 'linear-gradient(135deg,#00e5ff,#0055ff)', border: 'none', borderRadius: 8, padding: '10px 16px', color: '#03060f', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'Syne, sans-serif', flexShrink: 0 }}>
          Load
        </button>
      </div>
    ...
    <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 16, overflow: 'hidden' }}>
      ...
      {loading ? (
        ...
        balances.map(function(token) {
          ...
          return (
            <div key={token.mint} onClick={function() { onSelectToken && onSelectToken({ id: token.mint, symbol: token.symbol, name: token.name, image: token.logoURI, current_price: getPrice(token.symbol), isSolanaToken: true, mint: token.mint }); }} style={{ padding: '12px 16px', display: 'grid', gridTemplateColumns: '1fr 80px 80px 90px', gap: 8, alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,.025)', cursor: onSelectToken ? 'pointer' : 'default' }} onMouseEnter={function(e) { e.currentTarget.style.background = 'rgba(0,229,255,.02)'; }} onMouseLeave={function(e) { e.currentTarget.style.background = 'transparent'; }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        ...
        {!evmLoading && evmTokens.length > 0 && (
          <span style={{ fontSize: 11, color: C.muted }}>
            Total: <span style={{ color: C.green, fontWeight: 700 }}>{fmt(evmTokens.reduce(function(sum, t) { return sum + t.value; }, 0))}</span>
          </span>
        )}
      </div>
      <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 16, overflow: 'hidden' }}>
