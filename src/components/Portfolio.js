...
async function fetchOkxPrice(mint, decimals = 6, force = false) {
  ...
  try {
    ...
    if (j.code === '0' && j.data) {
      ...
      /*
        OKX returns USDC in base units.
        USDC has 6 decimals, so USD price = toTokenAmount / 1e6.
        Old code divided SOL by 1e9, which made SOL show around $0.09 instead of ~$90.
      */
  ...
  const fetchPortfolio = useCallback(async (force = false) => {
    ...
    try {
      ...
      for (const h of holdings) {
        ...
        /*
          Safety filter:
          Keep the token visible, but only count exact quote prices.
          If OKX cannot quote it, price/value stay zero instead of guessing by name or symbol.
        */
        priced.push({
          ...h,
