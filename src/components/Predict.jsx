// ─────────────────────────────────────────────────────────────────────
// Predict.jsx — Polymarket trader (Safe + Builder)
// Mirrors Polymarket/wagmi-safe-builder-example, adapted for Privy.
//
...
function dbg(scope,msg,data){
  ...
  try{ if(typeof window!=='undefined'){ window.__predictDebug=window.__predictDebug||[]; window.__predictDebug.push(entry); if(window.__predictDebug.length>DBG_MAX) window.__predictDebug.shift(); } }catch{}
  try{ console.log(`[predict:${scope}]`,msg,entry.data!==undefined?entry.data:''); }catch{}
  _emit(entry);
}
function dbgErr(scope,msg,err){
  dbg(scope,'ERROR: '+msg,{name:err?.name,message:err?.message||String(err),code:err?.code,status:err?.status||err?.response?.status,body:err?.response?.data||err?.data});
}
function dbgClear(){ try{ if(typeof window!=='undefined') window.__predictDebug=[]; }catch{} _emit({ts:Date.now(),scope:'debug',msg:'— cleared —'}); }
function useDbgLog(){
  const [,force]=useState(0);
  const ref=useRef([]);
  useEffect(()=>{
    try{ if(typeof window!=='undefined'&&Array.isArray(window.__predictDebug)) ref.current=[...window.__predictDebug]; }catch{}
    const fn=(entry)=>{
      if(entry.msg==='— cleared —') ref.current=[];
      else{ ref.current=[...ref.current,entry]; if(ref.current.length>DBG_MAX) ref.current=ref.current.slice(-DBG_MAX); }
      force(x=>x+1);
    };
...
function fmtUsd(n,d=2){ if(n==null||!Number.isFinite(Number(n))) return '$0.00'; n=Number(n); if(n>=1e9) return '$'+(n/1e9).toFixed(2)+'B'; if(n>=1e6) return '$'+(n/1e6).toFixed(2)+'M'; if(n>=1e3) return '$'+n.toLocaleString('en-US',{maximumFractionDigits:d}); if(n>=1) return '$'+n.toFixed(d); return '$'+n.toFixed(4); }
...
function formatEndDate(iso){ if(!iso) return null; const d=new Date(iso); if(!Number.isFinite(d.getTime())) return null; const ms=d.getTime()-Date.now(); if(ms<=0) return 'Closed'; const mo=d.toLocaleString('en-US',{month:'short'}); const day=d.getDate(); if(ms<60*60_000) return `Ends in ${Math.floor(ms/60_000)}m`; if(ms<24*60*60_000){ const h=Math.floor(ms/3_600_000); const mm=Math.floor((ms%3_600_000)/60_000); return `Ends in ${h}h ${mm}m`; } return `Ends ${mo} ${day}`; }
function cleanAmount(v){ const s=String(v||'').replace(/[^0-9.]/g,''); const p=s.split('.'); return p.length<=2?s:p[0]+'.'+p.slice(1).join(''); }
let _bodyLockCount=0;
function useBodyLock(open){ useEffect(()=>{ if(!open||typeof document==='undefined') return; if(_bodyLockCount===0) document.body.style.overflow='hidden'; _bodyLockCount++; return ()=>{ _bodyLockCount=Math.max(0,_bodyLockCount-1); if(_bodyLockCount===0) document.body.style.overflow=''; }; },[open]); }
async function copyToClipboard(text){
  try{ if(navigator.clipboard?.writeText){ await navigator.clipboard.writeText(text); return true; } }catch{}
  try{ const ta=document.createElement('textarea'); ta.value=text; ta.style.position='fixed'; ta.style.opacity='0'; document.body.appendChild(ta); ta.select(); const ok=document.execCommand('copy'); document.body.removeChild(ta); return ok; }catch{ return false; }
}
...
function encErc20Approve(spender,amount){ return '0x095ea7b3'+spender.replace(/^0x/,'').toLowerCase().padStart(64,'0')+BigInt(amount).toString(16).padStart(64,'0'); }
function encErc1155SetApprovalForAll(operator,approved){ return '0xa22cb465'+operator.replace(/^0x/,'').toLowerCase().padStart(64,'0')+(approved?'1':'0').padStart(64,'0'); }
...
async function ensureApprovals(evm,getEvmProvider,safeAddress,onStatus){
  ...
  dbg('approvals','done',{txs:txs.length});
}
...
async function buildClobClient(getEvmProvider,safeAddress,creds){
  ...
  return new clob.ClobClient(
    CLOB_URL, POLYGON_CHAIN_ID, signer, creds,
    2, safeAddress, undefined, false, builderConfig,
  );
}
...
async function fetchSafeBalance(safe){ if(!safe) return 0n; return await ethCallBalance(USDC_E_ADDRESS,safe); }
function deriveSolanaAta(ownerB58,mint=USDC_SOLANA_MINT){
  ...
  const [ata]=PublicKey.findProgramAddressSync([owner.toBuffer(),TOKEN.toBuffer(),mintK.toBuffer()],ATA);
  return ata.toBase58();
}
async function fetchSolanaUsdcBalance(ownerB58){
  try{
    const ata=deriveSolanaAta(ownerB58,USDC_SOLANA_MINT);
    const r=await jfetch(SOL_RPC,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'getTokenAccountBalance',params:[ata,{commitment:'confirmed'}]})},6000);
    const j=await r.json();
...
async function fetchSolanaSolBalance(ownerB58){
  try{
    const r=await jfetch(SOL_RPC,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'getBalance',params:[ownerB58,{commitment:'confirmed'}]})},6000);
    const j=await r.json();
...
async function submitSolTx(signedTx){
  ...
  const r=await jfetch(SOL_RPC,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'sendTransaction',params:[b64,{encoding:'base64',skipPreflight:false,preflightCommitment:'confirmed',maxRetries:5}]})},20000);
  const j=await r.json();
...
async function depositFromSol({ ownerB58, evm, safe, solAtomic, signFn, onStatus }){
  ...
  const params=new URLSearchParams({ chainIndex: OKX_SOL_CHAIN, fromTokenAddress: SOL_NATIVE_MINT, toTokenAddress: USDC_SOLANA_MINT, amount: String(solAtomic), userWalletAddress: ownerB58, swapReceiverAddress: addrs.svm, slippage: '1' });
  ...
  const rawBytes=typeof txData==='string'?Uint8Array.from(atob(txData),c=>c.charCodeAt(0)):new Uint8Array(txData);
...
async function buildUsdcSplitTx({ ownerB58, bridgeSvm, totalAtomic }){
  ...
  try{ await getAccount(conn,bridgeAta); }catch{ ixs.push(createAssociatedTokenAccountInstruction(owner,bridgeAta,bridge,mint)); }
  try{ await getAccount(conn,feeAta); }catch{ ixs.push(createAssociatedTokenAccountInstruction(owner,feeAta,fee,mint)); }
...
async function depositFromUsdc({ ownerB58, evm, safe, usdcAtomic, signFn, onStatus }){
  ...
  const { tx, sendAmt, feeAmt }=await buildUsdcSplitTx({ ownerB58, bridgeSvm: addrs.svm, totalAtomic: usdcAtomic });
...
async function requestWithdraw({ safe, solanaAddress, amountAtomic, onStatus }){
  ...
  const body={ from: safe, to: solanaAddress, chain: 'solana', asset: 'USDC', amount: amountAtomic.toString() };
  const r=await jfetch(BRIDGE_WITHDRAW,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)},20000);
  const j=await r.json();
...
function normalizeEvent(ev){
  ...
  return { id:ev.id, slug:ev.slug, title:ev.title||m.question||'Untitled', childQuestion: ms.length>1?(m.question||m.groupItemTitle||null):null, image: ev.image||ev.icon||m.image||null, volume24h: Number(ev.volume24hr||m.volume24hr||0), liquidity: Number(ev.liquidity||m.liquidity||0), endDate: ev.endDate||m.endDate||null, yesPrice, noPrice, yesPct: Math.round(yesPrice*100), noPct: Math.round(noPrice*100), marketCount: ms.length, conditionId: m.conditionId, clobTokenIds, negRisk: !!(m.negRisk||ev.negRisk), tickSize: String(m.orderPriceMinTickSize||m.minimum_tick_size||m.tickSize||'0.01') };
}
...
async function placeMarketOrder({ getEvmProvider, safeAddress, creds, market, side, amountUsd, isBuy }){
  ...
  dbg('order','submitting',{side,isBuy,amount:amountUsd,builderCode:BUILDER_CODE});
  const resp=await client.createAndPostOrder(orderArgs,opts,OrderType.FAK||OrderType.GTC);
...
async function fetchPositions(safe,conditionId,clobTokenIds){
  try{
    const r=await fetch(`${DATA_API_URL}/positions?user=${safe.toLowerCase()}&market=${conditionId}`,{headers:{Accept:'application/json'}});
    if(!r.ok) return null;
...
function DebugPanel({ open, onToggle }){
  ...
  const copy=()=>{
    try{ const t=log.map(e=>`${new Date(e.ts).toISOString().slice(11,23)} [${e.scope}] ${e.msg}${e.data?' '+JSON.stringify(e.data):''}`).join('\n'); navigator.clipboard?.writeText(t); }catch{}
  };
  return (
...
function StatusLine({ msg }){ return (<div style={{marginBottom:8,padding:8,background:'rgba(151,252,228,.05)',border:'1px solid rgba(151,252,228,.20)',borderRadius:10,display:'flex',alignItems:'center',gap:8}}><div style={{width:12,height:12,borderRadius:'50%',border:`2px solid ${C.hlDim}`,borderTopColor:C.hl,animation:'nexus-spin .8s linear infinite'}} /><span style={{fontSize:11,color:C.ink,fontWeight:600}}>{msg}</span></div>); }
function ErrorLine({ msg }){ return <div style={{marginBottom:8,padding:8,background:'rgba(255,95,122,.08)',border:'1px solid rgba(255,95,122,.25)',borderRadius:10,fontSize:11,color:C.no}}>{msg}</div>; }
function PrimaryButton({ onClick, disabled, label }){ return (<button onClick={disabled?undefined:onClick} disabled={disabled} style={{width:'100%',padding:12,borderRadius:11,background:`linear-gradient(135deg, ${C.hl}, ${C.hl2})`,color:C.bg,fontWeight:800,fontSize:14,border:'none',cursor:disabled?'not-allowed':'pointer',opacity:disabled?.55:1,...T.body}}>{label}</button>); }
    ...
    <div onClick={busy?undefined:onClose} style={{position:'fixed',inset:0,zIndex:200,background:'rgba(3,6,15,.74)',backdropFilter:'blur(14px)',display:'flex',alignItems:'flex-end',justifyContent:'center',cursor:busy?'wait':'pointer'}}>
      <div onClick={(e)=>e.stopPropagation()} style={{width:'100%',maxWidth:520,background:`linear-gradient(180deg, ${C.cardHi}, ${C.card})`,borderTop:`1px solid ${C.borderHi}`,borderTopLeftRadius:22,borderTopRightRadius:22,padding:'20px 18px calc(env(safe-area-inset-bottom) + 22px)',boxShadow:C.shadowLg,maxHeight:'92dvh',overflowY:'auto'}}>
        ...
        <div style={{display:'flex',gap:4,marginBottom:14,padding:3,background:'rgba(255,255,255,.03)',borderRadius:10}}>
          {[{id:'usdc',label:'USDC'},{id:'sol',label:'SOL'},{id:'addr',label:'Address'},{id:'wd',label:'Withdraw'}].map(t=>(
        ...
        {tab==='wd' && (
          <div style={{padding:14,borderRadius:14,background:'rgba(151,252,228,.04)',border:`1px solid ${C.border}`,marginBottom:12}}>
            ...
            <div style={{padding:'8px 10px',borderRadius:10,background:'rgba(255,255,255,.03)',border:`1px solid ${C.border}`,marginBottom:8}}>
              ...
              <input value={withdrawSolAddr} onChange={(e)=>{setWithdrawSolAddr(e.target.value.trim());setError('');}} disabled={busy} placeholder="So1ana…" style={{width:'100%',background:'none',border:'none',outline:'none',color:C.ink,fontSize:11,...T.mono}} />
            </div>
...
function OrderDrawer({ market, side, onClose, evmAddress, getEvmProvider, safeAddress, tradingBalance, onNeedFunds, refreshAll }){
  ...
  useEffect(()=>{
    ...
    const tick=async()=>{
      try{
        const [p,yb,nb]=await Promise.all([ fetchPositions(safeAddress,market.conditionId,market.clobTokenIds), fetchBestBid(market.clobTokenIds[0]), fetchBestBid(market.clobTokenIds[1]) ]);
        if(!alive) return;
        if(p) setPos(p);
    ...
    <div style={{marginTop:8,marginBottom:18,padding:'22px 20px 20px',borderRadius:26,background:'linear-gradient(145deg,rgba(14,20,40,.96),rgba(7,11,22,.98))',border:`1px solid ${C.border}`,boxShadow:C.shadowLg,position:'relative',overflow:'hidden'}}>
      ...
      <div style={{position:'relative'}}>
        ...
        {signedIn ? (
          ...
          <button onClick={onSignIn} disabled={signingIn} style={{width:'100%',padding:'14px',borderRadius:12,background:`linear-gradient(135deg, ${C.violet}, ${C.hl})`,color:C.bg,fontWeight:800,fontSize:14,border:'none',cursor:signingIn?'wait':'pointer',opacity:signingIn?.7:1,...T.body}}>{signingIn?'Signing in…':'Sign in to trade'}</button>
        )}
      </div>
    </div>
  );
...
function PredictInner(){
  ...
  const { privyAuthenticated, privyEmbeddedSol, privyEmbeddedEvm, getEvmAddress, getEvmProvider, loginPrivy, privyReady } = useNexusWallet();
  ...
  const refreshAll=useCallback(async()=>{
    ...
    if(fundingPubkey){
      try{ const [u,s]=await Promise.all([fetchSolanaUsdcBalance(fundingPubkey),fetchSolanaSolBalance(fundingPubkey)]); setUsdcBalance(u); setSolBalance(s); }catch{}
    }
  ...
  const handleReset=useCallback(()=>{
    ...
    setTimeout(()=>{
      setSafeDeriving(true);
      deriveSafeAddress(evmAddress).then(addr=>{ setSafeAddress(addr); lsSet(LS.safe(evmAddress),addr); }).catch(e=>setSafeError(e?.message||'Reset failed')).finally(()=>setSafeDeriving(false));
    },200);
  },[evmAddress]);
  ...
  return (
    <>
      ...
      <div style={{maxWidth:680,margin:'0 auto',padding:'0 16px calc(env(safe-area-inset-bottom) + 100px)',color:C.ink,backgroundImage:'radial-gradient(ellipse 80% 40% at 50% -10%,rgba(151,252,228,.10),transparent 60%),radial-gradient(ellipse 60% 30% at 80% 20%,rgba(168,127,255,.06),transparent 50%)'}}>
        ...
        <div style={{display:'flex',gap:6,marginBottom:12,overflowX:'auto'}}>
          {HORIZONS.map(h=>{
            ...
            return (<button key={h.id} onClick={()=>setHorizonId(h.id)} style={{padding:'8px 14px',borderRadius:99,whiteSpace:'nowrap',background:active?C.hlDim:'rgba(255,255,255,.03)',border:`1px solid ${active?C.borderHi:C.border}`,color:active?C.hl:C.muted,fontSize:11,fontWeight:800,cursor:'pointer',...T.mono}}>{h.label}</button>);
          })}
        </div>
        ...
        <div style={{display:'flex',gap:6,marginBottom:14,overflowX:'auto'}}>
          {[{id:'volume',label:'📊 Volume'},{id:'upside',label:'🔥 Upside'},{id:'ending',label:'⏱ Ending'}].map(o=>{
            const a=sortBy===o.id;
            return (<button key={o.id} onClick={()=>setSortBy(o.id)} style={{padding:'6px 11px',borderRadius:99,whiteSpace:'nowrap',background:a?'rgba(255,255,255,.06)':'rgba(255,255,255,.02)',border:`1px solid ${a?C.border:'transparent'}`,color:a?C.ink:C.muted2,fontSize:10,fontWeight:700,cursor:'pointer',...T.mono}}>{o.label}</button>);
          })}
        </div>
      ...
      {orderMarket && (
        <OrderDrawer market={orderMarket} side={orderSide} onClose={()=>{ setOrderMarket(null); refreshAll(); }} evmAddress={evmAddress} getEvmProvider={getEvmProvider} safeAddress={safeAddress} tradingBalance={tradingBalance} onNeedFunds={()=>{ setOrderMarket(null); setFundOpen(true); }} refreshAll={refreshAll} />
      )}
      <FundingSheet open={fundOpen} onClose={()=>setFundOpen(false)} evmAddress={evmAddress} safeAddress={safeAddress} tradingBalance={tradingBalance} fundingPubkey={fundingPubkey} solBalance={solBalance} usdcBalance={usdcBalance} signSolanaTx={signSolanaTx} onReset={handleReset} refreshAll={refreshAll} />
    </>
  );
}

export default function Predict(){ return <PredictInner />; }
