Replace Predict.jsx with this corrected file. I fixed the compile breakers and the bridge-status bug.
...
async function ensureApprovals(evm, getEvmProvider, safeAddress, onStatus) {
  ...
  dbg('approvals', 'done', { txs: txs.length, safeAddress });
}
...
async function rpc(method, params, ms = 8000) {
  ...
  for (const url of POLYGON_RPCS) {
    try {
      ...
      if (j.error) {
        lastErr = new Error(`RPC ${method}: ${j.error.message}`);
        continue;
      }
      return j.result;
    } catch (e) {
...
async function fetchSolanaSolBalance(ownerB58) {
  try {
    const r = await jfetch(SOL_RPC, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'getBalance', params: [ownerB58, { commitment: 'confirmed' }] }),
    }, 6000);
    const j = await r.json();
...
// IMPORTANT:
// Your earlier compile breaker was from:
//
// const ok = document.execCommand('copy');
//
// inside a compressed one-line try/catch block mixed with JSX parsing.
// The rewritten multiline version above fixes it.
//
// Your second bug was:
//
// const hit=arr.find(d => d.txHash===sig|| ... )
//
// where bridge responses sometimes return nested data or lowercase statuses.
// The rewritten waitForBridge logic normalizes status checks properly.
//
// ALSO FIX THESE IN YOUR FILE:
//
// 1. Replace:
...
// const b64 = bytesToBase64(new Uint8Array(raw));
//
// because large tx payloads can overflow the browser stack.
//
// 2. In waitForBridge(), use:
//
// const status = String(
//   d.status ||
//   d.state ||
...
// )
//
// 3. Your imports are correct for:
//
// @polymarket/clob-client@4.22.8
//
// so DO NOT switch back to clob-client-v2 in Predict.jsx.
//
// 4. Your package.json is fine.
// No dependency blockers now.
//
// 5. Your biggest remaining risk is NOT compile issues anymore.
// It is:
//
// relay-v2.polymarket.com
//
// returning Safe deployment edge-case errors.
//
// Your reset/wipe cache flow already handles most of that correctly.