      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ padding: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 18 }}>
              {['Visa', 'Mastercard', 'Apple Pay', 'Google Pay', 'Bank Transfer'].map((method) => (
                <div key={method} style={{ background: 'rgba(0,229,255,.06)', border: '1px solid rgba(0,229,255,.15)', borderRadius: 6, padding: '4px 10px', fontSize: 11, color: C.accent, fontWeight: 600 }}>
                  {method}
                </div>
              ))}
            </div>
        ...
        <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 20, overflow: 'hidden' }}>
          <div style={{ padding: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 18 }}>
              {['Visa', 'Mastercard', 'Apple Pay', 'Google Pay', 'Bank Transfer'].map((method) => (
                <div key={method} style={{ background: 'rgba(100,60,255,.08)', border: '1px solid rgba(100,60,255,.2)', borderRadius: 6, padding: '4px 10px', fontSize: 11, color: '#a78bfa', fontWeight: 600 }}>
                  {method}
                </div>
              ))}
            </div>
