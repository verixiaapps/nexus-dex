  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
    ...
    <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 20, overflow: 'hidden' }}>
      <div style={{ padding: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 18 }}>
          {['Visa', 'Mastercard', 'Apple Pay', 'Google Pay', 'Bank Transfer'].map(function(method) {
            return <div key={method} style={{ background: 'rgba(100,60,255,.08)', border: '1px solid rgba(100,60,255,.2)', borderRadius: 6, padding: '4px 10px', fontSize: 11, color: '#a78bfa', fontWeight: 600 }}>{method}</div>;
          })}
        </div>
        <a href={'https://buy.moonpay.com?currencyCode=' + coin + (walletAddress ? '&walletAddress=' + encodeURIComponent(walletAddress) : '')} target="_blank" rel="noreferrer" style={{ display: 'block', width: '100%', padding: 15, background: 'linear-gradient(135deg,#6b3fff,#a855f7)', borderRadius: 12, color: '#fff', fontWeight: 800, fontSize: 15, textAlign: 'center', textDecoration: 'none', fontFamily: 'Syne, sans-serif' }}>
          Buy with MoonPay
        </a>
