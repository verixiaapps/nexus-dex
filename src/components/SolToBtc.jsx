...
* Quotes work without a wallet connected (preview pricing).
*/
...
// Placeholder sender for quote-only requests (no wallet connected).
// LI.FI requires a fromAddress to return a quote; this is a known SOL address.
const QUOTE_PLACEHOLDER_SENDER = WSOL_MINT;
...
export default function SolToBtc({ onConnectWallet }) {
 ...
 /* QUOTE — works without wallet connected (preview pricing) */
 const fetchQuote = useCallback(async () => {
   ...
   try {
     ...
     // Use connected wallet if available, otherwise placeholder for quote-only
     const sender = pubkey ? pubkey.toString() : QUOTE_PLACEHOLDER_SENDER;
 ...
 const execute = useCallback(async () => {
   ...
   try {
     ...
     // Always refetch with real wallet + real BTC address before executing
     // (the preview quote may have used a placeholder sender or address)
     setStatusMsg('Finalizing route with your wallet…');
 ...
 const btnLabel = () => {
   if (!wcon) {
     if (!solAmt) return 'Enter Amount';
     return 'Connect Wallet to Bridge';
   }
