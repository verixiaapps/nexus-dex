 ...
 *
 * Locked pair: SOL or USDC on Solana → native BTC on Bitcoin.
 * Same atomic single-tx flow as CrossChain.jsx (5% fee in SOL).
 */
...
const BTC_TOKEN = {
  chainId:  String(LIFI_BITCOIN_ID),
  address:  BTC_TOKEN_ID,
  symbol:   'BTC',
  name:     'Bitcoin',
...
const lifiQuote = async ({ fromMint, amount, sender, btcAddress }) => {
...
export default function SolToBtc({ onConnectWallet }) {
  ...
  const [fromToken, setFromToken] = useState(FROM_SOL);
  ...
  const fetchQuote = useCallback(async () => {
    setQuoteErr('');
    if (!fromAmt || +fromAmt <= 0) { setQuote(null); return; }
  ...
  }, [fromAmt, fromToken, btcAddr, pubkey, solPrice]);
  ...
  const execute = useCallback(async () => {
    ...
    wcon, btcAddr, fromToken, fromAmt, pubkey, signTransaction,
  ...
  const btnLabel = () => {
    ...
    return `Bridge ${fromToken?.symbol || ''} → BTC`;
  };
  const btnDisabled = busy ||
    (wcon && (!fromAmt || !btcAddr.trim() || !!addrErr ||
    ...
    <div className="sb-page">
      <div className="sb-header">
        ...
        <div className="sb-pair">
          <TokenIcon token={FROM_SOL} size={44}/>
        ...
        <p className="sb-subtitle">
          Swap SOL or USDC for native BTC · powered by LI.FI
        </p>
      </div>
      ...
      <div className="sb-card">
        ...
        <div className="sb-io-box">
            ...
            <div className="sb-io-meta">
              ...
              {fbd != null && (
                <span className="sb-io-bal">
                  Bal: <span className="sb-io-bal-val">{fmtTok(fbd)}</span>
                </span>
              )}
            </div>
          </div>
          ...
          <div className="sb-toggle">
            {[FROM_SOL, FROM_USDC].map(t => {
              const active = fromToken.mint === t.mint;
              return (
                <button
                  key={t.mint}
                  onClick={() => {
                  ...
                  disabled={busy}
                  className={'sb-toggle-btn' + (active ? ' sb-toggle-btn-active' : '')}
                >
                  <TokenIcon token={t} size={22}/>
                  <span className="sb-toggle-sym">{t.symbol}</span>
                </button>
              );
            })}
          </div>
